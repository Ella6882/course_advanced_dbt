...
{% set subscriptions = unit_testing_select_table(ref('dim_subscriptions'), ref('unit_test_input_dim_subscriptions')) %}
{% set dates = unit_testing_select_table(ref('dim_dates'), ref('unit_test_input_dim_dates')) %}
...
monthly_subscriptions AS (
    SELECT
        ...
        plan_name,
        pricing,
        {{ truncate_to_date('starts_at') }} AS start_month,
        {{ truncate_to_date('ends_at') }} AS end_month
    FROM
        {{ subscriptions }}
    WHERE
...
subscription_periods AS (
    SELECT
        ...
        CASE
            ...
            WHEN end_month IS NULL THEN DATEADD('month', 1, {{ truncate_to_date() }})
            ELSE end_month
        END AS end_month
    FROM
